...
from binance_quant_control.analysis import build_trade_plan, enrich_indicators, prepare_klines_frame
from binance_quant_control.backtest import simulate_backtest
from binance_quant_control.config import REPORTS_DIR, ensure_runtime_dirs
...
LONG_THRESHOLDS = [65, 70, 75]
MIN_CONVERGENCES = [0.6, 0.7]
ATR_MULTIPLES = [1.8, 2.5]
TP_MULTIPLES = [0.75, 1.0, 1.5, 2.0]
...
def fetch_klines_paginated(symbol: str, interval: str, total: int = TOTAL_CANDLES) -> list[list[Any]]:
    url = f"{base_url('futures')}/fapi/v1/klines"
...
def run_one(df, strategy) -> dict[str, Any]:
    summary = simulate_backtest(df, "futures", strategy)
    return {**summary, **summary_stats(summary)}
...
def main() -> None:
    ...
    output_dir = REPORTS_DIR / f"{run_id}-volatile-targeted-sweep"
    ...
    for symbol in SYMBOLS:
        rows = fetch_klines_paginated(symbol, INTERVAL, TOTAL_CANDLES)
        ...
        for long_threshold, min_conv, atr_mult, tp_mult in itertools.product(
            ...
            full = run_one(df, strategy)
            train = run_one(train_df, strategy)
            test = run_one(test_df, strategy)
            latest = df.iloc[-1]
            plan = build_trade_plan(latest, test.get("bias", "neutral"), "futures", strategy)
            results.append(
                {
                    "symbol": symbol,
                    "params": {
                    ...
                    "latest": {
                        ...
                        "tp1": plan["long"]["take_profit_1"],
                        "stop": plan["long"]["invalidation"],
                        "tp1_pct": round((float(plan["long"]["take_profit_1"]) - float(latest["close"])) / float(latest["close"]) * 100, 4),
                    },
                }
            )
    ...
    for symbol, rows in by_symbol.items():
        best = rows[0]
        required_leverage_for_1p5 = 1.5 / (2.0 * (best["latest"]["tp1_pct"] / 100.0)) if best["latest"]["tp1_pct"] > 0 else float("inf")
        best_per_symbol.append(
            {
                "symbol": symbol,
                "best": best,
                "required_leverage_for_1p5_on_2usdt_margin": round(required_leverage_for_1p5, 2),
            }
        )
    ...
    aggregate = {
        ...
        "zero_loss_test_count": sum(1 for r in ranked if r["test"]["all_trades_positive"]),
        ...
        "mean_tp1_pct": round(mean(r["latest"]["tp1_pct"] for r in results), 4) if results else 0.0,
    }
    ...
    print(json.dumps({"report_path": str(report_path), "aggregate": aggregate}, indent=2, ensure_ascii=False))
